...
function loadDreams() {
  const dreamList = document.getElementById('dreamList');
  const raw = JSON.parse(localStorage.getItem('dreamJournal')) || [];
  // filter out any non-objects or malformed entries
  const dreams = raw.filter(d => d && typeof d.text === 'string' && d.timestamp);

  if (dreams.length === 0) {
    dreamList.innerHTML = `
      <li class="default-message glow">No dreams recorded yet</li>
    `;
    return;
  }
...
document.getElementById('clearDreamsButton').addEventListener('click', () => {
  if (confirm('❗ Are you sure you want to delete all saved dreams?')) {
